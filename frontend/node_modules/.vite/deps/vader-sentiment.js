    ...
    exports.SentimentIntensityAnalyzer = function(e) {
      ...
      var r = /* @__PURE__ */ function() {
        ...
        return function(s2, i2, r2) {
          return i2 && e2(s2.prototype, i2), r2 && e2(s2, r2), s2;
        };
      }();
      function t(e2, s2) {
        if (!(e2 instanceof s2)) throw new TypeError("Cannot call a class as a function");
      }
      var n = i(0).lexicon, a = s.B_INCR = 0.293, l = s.B_DECR = -0.293, o = s.C_INCR = 0.733, d = s.N_SCALAR = -0.74, u = s.REGEX_REMOVE_PUNCTUATION = new RegExp(/[!"#$%&'()*+,-./:;<=>?@[\\\]^_`{|}~]/g), c = s.PUNC_LIST = [".", "!", "?", ",", ";", ":", "-", "'", '"', "!!", "!!!", "??", "???", "?!?", "!?!", "?!?!", "!?!?"], g = s.NEGATE = ["aint", "arent", "cannot", "cant", "couldnt", "darent", "didnt", "doesnt", "ain't", "aren't", "can't", "couldn't", "daren't", "didn't", "doesn't", "dont", "hadnt", "hasnt", "havent", "isnt", "mightnt", "mustnt", "neither", "don't", "hadn't", "hasn't", "haven't", "isn't", "mightn't", "mustn't", "neednt", "needn't", "never", "none", "nope", "nor", "not", "nothing", "nowhere", "oughtnt", "shant", "shouldnt", "uhuh", "wasnt", "werent", "oughtn't", "shan't", "shouldn't", "uh-uh", "wasn't", "weren't", "without", "wont", "wouldnt", "won't", "wouldn't", "rarely", "seldom", "despite"], m = s.BOOSTER_DICT = { absolutely: a, amazingly: a, awfully: a, completely: a, considerably: a, decidedly: a, deeply: a, effing: a, enormously: a, entirely: a, especially: a, exceptionally: a, extremely: a, fabulously: a, flipping: a, flippin: a, fricking: a, frickin: a, frigging: a, friggin: a, fully: a, fucking: a, greatly: a, hella: a, highly: a, hugely: a, incredibly: a, intensely: a, majorly: a, more: a, most: a, particularly: a, purely: a, quite: a, really: a, remarkably: a, so: a, substantially: a, thoroughly: a, totally: a, tremendously: a, uber: a, unbelievably: a, unusually: a, utterly: a, very: a, almost: l, barely: l, hardly: l, "just enough": l, "kind of": l, kinda: l, kindof: l, "kind-of": l, less: l, little: l, marginally: l, occasionally: l, partly: l, scarcely: l, slightly: l, somewhat: l, "sort of": l, sorta: l, sortof: l, "sort-of": l }, p = s.SPECIAL_CASE_IDIOMS = { "the shit": 3, "the bomb": 3, "bad ass": 1.5, "yeah right": -2, "cut the mustard": 2, "kiss of death": -1.5, "hand to mouth": -2 }, h = s.negated = function(e2) {
        var s2 = !(arguments.length > 1 && void 0 !== arguments[1]) || arguments[1], i2 = [];
        i2.push.apply(i2, g);
        for (var r2 = 0; r2 < i2.length; r2++) if (e2.indexOf(i2[r2]) >= 0) return true;
        if (true === s2) {
          for (var t2 = 0; t2 < e2.length; t2++) if (e2[t2].indexOf("n't") >= 0) return true;
        }
        var n2 = e2.findIndex(function(e3) {
          return "least" === e3;
        });
        return -1 !== n2 && n2 > 0 && "at" !== e2[n2 - 1];
      }, f = s.normalize = function(e2) {
        var s2 = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : 15, i2 = e2 / Math.sqrt(e2 * e2 + s2);
        return i2 < -1 ? -1 : i2 > 1 ? 1 : i2;
      }, y = s.allcap_differential = function(e2) {
        for (var s2 = 0, i2 = 0; i2 < e2.length; i2++) v(e2[i2]) && (s2 += 1);
        var r2 = e2.length - s2;
        return r2 > 0 && r2 < e2.length;
      }, b = s.scalar_inc_dec = function(e2, s2, i2) {
        var r2 = 0, t2 = e2.toLowerCase();
        return m.hasOwnProperty(t2) && (r2 = m[t2], s2 < 0 && (r2 *= -1), i2 && v(e2) && (s2 > 0 ? r2 += o : r2 -= o)), r2;
      }, v = s.is_upper_python = function(e2) {
        return ("string" == typeof e2 || e2 instanceof String) && e2.length > 0 && /^[^a-z]*[A-Z]+[^a-z]*$/g.test(e2);
      }, w = s.SentiText = function() {
        function e2(s2) {
          t(this, e2), this.text = s2, this.words_and_emoticons = this.get_words_and_emoticons(), this.is_cap_diff = y(this.words_and_emoticons);
        }
        return r(e2, [{ key: "get_words_plus_punc", value: function() {
          for (var e3 = this.text.slice(0).replace(u, "").split(/\s/).filter(function(e4) {
            return e4.length > 1;
          }), s2 = {}, i2 = 0; i2 < c.length; i2++) for (var r2 = 0; r2 < e3.length; r2++) {
            var t2 = "" + c[i2] + e3[r2], n2 = "" + e3[r2] + c[i2];
            s2[t2] = e3[r2], s2[n2] = e3[r2];
          }
          return s2;
        } }, { key: "get_words_and_emoticons", value: function() {
          for (var e3 = this.text.split(/\s/), s2 = this.get_words_plus_punc(), i2 = e3.filter(function(e4) {
            return e4.length > 1;
          }), r2 = 0; r2 < i2.length; r2++) s2.hasOwnProperty(i2[r2]) && (i2[r2] = s2[i2[r2]]);
          return i2;
        } }]), e2;
      }();
      s.SentimentIntensityAnalyzer = function() {
        ...
        return r(e2, null, [{ key: "polarity_scores", value: function(s2) {
          for (var i2 = new w(s2), r2 = [], t2 = i2.words_and_emoticons, n2 = 0; n2 < t2.length; n2++) {
            var a2 = t2[n2];
            n2 < t2.length - 1 && "kind" === a2.toLowerCase() && "of" === t2[n2 + 1].toLowerCase() || m.hasOwnProperty(a2.toLowerCase()) ? r2.push(0) : r2 = e2.sentiment_valence(0, i2, a2, n2, r2);
          }
          return r2 = e2.but_check(t2, r2), e2.score_valence(r2, s2);
        } }, { key: "sentiment_valence", value: function(e3, s2, i2, r2, t2) {
          var a2 = s2.is_cap_diff, l2 = s2.words_and_emoticons, d2 = i2.toLowerCase();
          if (n.hasOwnProperty(d2)) {
            e3 = n[d2], v(i2) && a2 && (e3 > 0 ? e3 += o : e3 -= o);
            for (var u2 = 0; u2 < 3; u2++) if (r2 > u2 && false === n.hasOwnProperty(l2[r2 - (u2 + 1)].toLowerCase())) {
              var c2 = b(l2[r2 - (u2 + 1)], e3, a2);
              1 === u2 && 0 !== c2 ? c2 *= 0.95 : 2 === u2 && 0 !== c2 && (c2 *= 0.9), e3 += c2, e3 = this.never_check(e3, l2, u2, r2), 2 === u2 && (e3 = this.idioms_check(e3, l2, r2));
            }
            e3 = this.least_check(e3, l2, r2);
          }
          return t2.push(e3), t2;
        } }, { key: "least_check", value: function(e3, s2, i2) {
          return i2 > 1 && "least" === s2[i2 - 1].toLowerCase() && false === n.hasOwnProperty(s2[i2 - 1].toLowerCase()) ? "at" !== s2[i2 - 2].toLowerCase() && "very" !== s2[i2 - 2].toLowerCase() && (e3 *= d) : i2 > 0 && "least" === s2[i2 - 1].toLowerCase() && false === n.hasOwnProperty(s2[i2 - 1].toLowerCase()) && (e3 *= d), e3;
        } }, { key: "but_check", value: function(e3, s2) {
          var i2 = e3.indexOf("but");
          if (-1 === i2 && (i2 = e3.indexOf("BUT")), -1 !== i2) for (var r2 = 0; r2 < s2.length; r2++) {
            var t2 = r2, n2 = s2[t2];
            t2 < i2 ? (s2.splice(t2, 1), s2.splice(t2, 0, 0.5 * n2)) : t2 > i2 && (s2.splice(t2, 1), s2.splice(t2, 0, 1.5 * n2));
          }
          return s2;
        } }, { key: "idioms_check", value: function(e3, s2, i2) {
          for (var r2 = s2[i2 - 1] + " " + s2[i2], t2 = s2[i2 - 2] + " " + s2[i2 - 1] + " " + s2[i2], n2 = s2[i2 - 2] + " " + s2[i2 - 1], a2 = s2[i2 - 3] + " " + s2[i2 - 2] + " " + s2[i2 - 1], o2 = s2[i2 - 3] + " " + s2[i2 - 2], d2 = [r2, t2, n2, a2, o2], u2 = 0; u2 < d2.length; u2++) if (p.hasOwnProperty(d2[u2])) {
            e3 = p[d2[u2]];
            break;
          }
          if (s2.length - 1 > i2) {
            var c2 = s2[i2] + " " + s2[i2 + 1];
            p.hasOwnProperty(c2) && (e3 = p[c2]);
          }
          if (s2.length - 1 > i2 + 1) {
            var g2 = s2[i2] + " " + s2[i2 + 1] + " " + s2[i2 + 2];
            p.hasOwnProperty(g2) && (e3 = p[g2]);
          }
          return (m.hasOwnProperty(o2) || m.hasOwnProperty(n2)) && (e3 += l), e3;
        } }, { key: "never_check", value: function(e3, s2, i2, r2) {
          return 0 === i2 && h([s2[r2 - 1]]) && (e3 *= d), 1 === i2 && ("never" !== s2[r2 - 2] || "so" !== s2[r2 - 1] && "this" !== s2[r2 - 1] ? h([s2[r2 - (i2 + 1)]]) && (e3 *= d) : e3 *= 1.5), 2 === i2 && ("never" === s2[r2 - 3] && ("so" === s2[r2 - 2] || "this" === s2[r2 - 2]) || "so" === s2[r2 - 1] || "this" === s2[r2 - 1] ? e3 *= 1.25 : h([s2[r2 - (i2 + 1)]]) && (e3 *= d)), e3;
        } }, { key: "punctuation_emphasis", value: function(s2, i2) {
          return e2.amplify_ep(i2) + e2.amplify_qm(i2);
        } }, { key: "amplify_ep", value: function(e3) {
          var s2 = e3.replace(/[^!]/g, "").length;
          return s2 > 4 && (s2 = 4), 0.292 * s2;
        } }, { key: "amplify_qm", value: function(e3) {
          var s2 = e3.replace(/[^?]/g, "").length, i2 = 0;
          return s2 > 1 && (i2 = s2 <= 3 ? 0.18 * s2 : 0.96), i2;
        } }, { key: "sift_sentiment_scores", value: function(e3) {
          for (var s2 = 0, i2 = 0, r2 = 0, t2 = 0; t2 < e3.length; t2++) {
            var n2 = e3[t2];
            n2 > 0 ? s2 += n2 + 1 : n2 < 0 ? i2 += n2 - 1 : r2 += 1;
          }
          return [s2, i2, r2];
        } }, { key: "score_valence", value: function(s2, i2) {
          if (s2 && s2.length > 0) {
            for (var r2 = 0, t2 = 0; t2 < s2.length; t2++) r2 += s2[t2];
            var n2 = e2.punctuation_emphasis(r2, i2);
            r2 > 0 ? r2 += n2 : r2 < 0 && (r2 -= n2);
            var a2 = f(r2), l2 = e2.sift_sentiment_scores(s2), o2 = l2[0], d2 = l2[1], u2 = l2[2];
            o2 > Math.abs(d2) ? o2 += n2 : o2 < Math.abs(d2) && (d2 -= n2);
            var c2 = o2 + Math.abs(d2) + u2, g2 = Math.abs(o2 / c2), m2 = Math.abs(d2 / c2), p2 = Math.abs(u2 / c2);
            return { neg: parseFloat(m2.toFixed(3)), neu: parseFloat(p2.toFixed(3)), pos: parseFloat(g2.toFixed(3)), compound: parseFloat(a2.toFixed(4)) };
          }
...
export default require_vaderSentiment_bundle();
//# sourceMappingURL=vader-sentiment.js.map
